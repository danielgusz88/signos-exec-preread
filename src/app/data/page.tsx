    ...
    <div className="p-6 pb-20">
      ...
      {loading && !status && (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-sm text-zinc-400">Checking integrations...</span>
        </div>
      )}
      ...
      {status && (
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <Card>
              <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Total Integrations</p>
              <p className="mt-1.5 text-3xl font-bold text-white">{status.summary.total}</p>
            </Card>
            <Card className={status.summary.configured > 0 ? 'border-emerald-500/10' : ''}>
              <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Configured</p>
              <p className="mt-1.5 text-3xl font-bold text-emerald-400">{status.summary.configured}</p>
            </Card>
            <Card>
              <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Not Configured</p>
              <p className="mt-1.5 text-3xl font-bold text-zinc-500">{status.summary.notConfigured}</p>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {status.integrations.map((source) => (
              <Card
                key={source.key}
                className={cn(
                  source.configured ? 'border-emerald-500/10' : 'border-white/[0.04]'
                )}
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <Server
                        className={cn(
                          'h-5 w-5',
                          source.configured ? 'text-emerald-400' : 'text-zinc-500'
                        )}
                      />
                    </div>
                    <div>
                ...
                {source.envVars.length > 0 && (
                  <div className="mb-3">
                    <p className="text-[10px] text-zinc-600 uppercase font-medium mb-1">Environment Variables</p>
                ...
                {/* Data provided */}
                <div>
                  <p className="text-[10px] text-zinc-600 uppercase font-medium mb-1">Data Provided</p>
                  <div className="flex flex-wrap gap-1">
                    {source.dataProvided.map((d) => (
                      <span
                        key={d}
                        className="rounded bg-white/[0.03] px-1.5 py-0.5 text-[10px] text-zinc-500 border border-white/[0.03]"
                      >
                        {d}
                      </span>
                    ))}
                ...
                {!source.configured && (
                  <div className="mt-4 rounded-lg bg-white/[0.02] p-3 border border-white/[0.04] text-center">
                    <a
                      href="/settings"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-lg bg-white/[0.02] p-4 border border-white/[0.04]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-white">Ingestion Layer</h4>
                </div>
                <ul className="space-y-2 text-xs text-zinc-400">
                  {status.integrations.map((source) => (
                    <li key={source.key} className="flex items-center gap-2">
                      ...
                      <span className={source.configured ? 'text-zinc-300' : 'text-zinc-500'}>
                        {source.name}
                      </span>
                    </li>
                  ))}
              ...
              <div className="rounded-lg bg-white/[0.02] p-4 border border-white/[0.04]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-white">Processing Layer</h4>
                </div>
                <ul className="space-y-2 text-xs text-zinc-400">
              ...
              <div className="rounded-lg bg-white/[0.02] p-4 border border-white/[0.04]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-white">Intelligence Layer</h4>
                </div>
                <ul className="space-y-2 text-xs text-zinc-400">
